...
def _complex_normal_np(shape: Tuple[int, ...], dtype: np.dtype) -> np.ndarray:
    z = (np.random.standard_normal(shape) + 1j * np.random.standard_normal(shape)) / np.sqrt(2.0)
    return z.astype(dtype)


def _path_gain_from_distance(d_m: np.ndarray, fc_ghz: float, shadow_std_db: float = 6.0) -> np.ndarray:
    d = np.maximum(d_m, 1.0)
    pl_db = 32.4 + 20.0 * np.log10(max(float(fc_ghz), 1e-3)) + 31.9 * np.log10(d)
    if shadow_std_db > 0.0:
        pl_db = pl_db + np.random.normal(loc=0.0, scale=float(shadow_std_db), size=d.shape)
...
def _optional_fs_specs(cfg: ResolvedConfig, L: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    csvs = sorted((repo_root / "data").glob("*.csv")) if (repo_root / "data").exists() else []
    tone_centers = np.linspace(0.0, 1.0, max(L, 1), endpoint=False, dtype=np.float32)
    i_max_dbm = np.full((L,), float(cfg.raw.get("fixed_service", {}).get("i_max_dbm", -110.0)), dtype=np.float32)
    if not csvs:
    ...
    try:
        ...
        if len(df) > 0:
            ...
            for key in ["i_max_dbm", "imax_dbm", "threshold_dbm", "protection_dbm", "rx_threshold_dbm"]:
                if key in numeric:
            ...
            for key in ["center_fraction", "center_norm", "fc_frac"]:
                if key in numeric:
                    vals = np.mod(rows[numeric[key]].astype(float).to_numpy(dtype=np.float32), 1.0)
                    tone_centers = vals
                    break
    except Exception:
        pass
...
def generate_ue_channels(cfg: ResolvedConfig, topo: TopologyData, batch_size: int = 1) -> tf.Tensor:
    ...
    fc_ghz = float(cfg.derived.get("carrier_frequency_ghz", cfg.raw["channel_model"].get("carrier_frequency_ghz", 7.0)))
    ...
    H = _complex_normal_np((S, B, U, Nr, M), np.complex64 if complex_dtype == tf.complex64 else np.complex128)
...
def generate_fs_stats(cfg: ResolvedConfig, topo: TopologyData, fs_loc: FixedServiceLocations, batch_size: int = 1) -> FsStats:
    real_dtype, complex_dtype = _precision_dtypes(cfg)
    S = int(batch_size)
    B = int(cfg.derived["num_bs"])
    ...
    d = np.linalg.norm(diff, axis=-1)
    bar_beta = _path_gain_from_distance(d, fc_ghz=fc_ghz, shadow_std_db=max(0.5 * shadow_std, 0.0))
    ...
    epsilon = np.exp(-0.5 * ((tone_axis - tone_centers[None, :]) / width) ** 2).astype(np.float32)
    ...
    i_max_watt = (1e-3 * (10.0 ** ((i_max_dbm - 30.0) / 10.0))).astype(np.float32)

    correlation = str(cfg.raw.get("antenna", {}).get("correlation", "identity"))
    a_bs_fs = None
    if correlation == "steering_rank1":
        ang = np.arctan2(fs[:, None, :, 1] - bs[:, :, None, 1], fs[:, None, :, 0] - bs[:, :, None, 0]).astype(np.float32)
        a_bs_fs = _steering_ula(ang, num_ant=M, dtype=np.complex64 if complex_dtype == tf.complex64 else np.complex128)
